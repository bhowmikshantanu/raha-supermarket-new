...
import {
  ...
  subscribeToCustomerCoupons,
  validateCustomerCoupon,
  type CustomerCoupon,
} from "@/src/services/firebaseCustomerCoupons";
import {
...
export default function Checkout() {
  ...
  const [liveCoupons, setLiveCoupons] = useState<CustomerCoupon[]>([]);
  ...
  useEffect(() => {
    const unsubscribe = subscribeToCustomerCoupons(
      (coupons) => {
        setLiveCoupons(coupons);
        setCouponsLoading(false);
      },
      (error) => {
        console.error("Unable to load live coupons:", error);
        setCouponsLoading(false);
      },
    );
  ...
  useEffect(() => {
    ...
    const result = validateCustomerCoupon(
      liveCoupons,
      appliedCoupon.code,
      cartSubtotal,
      deliveryFee,
  ...
  const handleApplyCoupon = (codeOverride?: string) => {
    ...
    if (couponsLoading) {
      showToast("Loading live coupons. Please try again.", "info");
      return;
    }
